...
const StatsCard: React.FC<StatsCardProps> = ({ stats }) => {
  ...
  return (
    <div className="bg-white rounded-xl shadow-md p-6">
      <h3 className="text-xl font-bold text-primary font-rubik mb-4">{t('profile.yourStats')}</h3>
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-4 flex flex-col items-center justify-center">
          <span className="text-gray-500 text-sm">{t('profile.rank')}</span>
          <span className="text-2xl font-bold text-gray-800 font-rubik">{stats.rank}</span>
        </div>
        
        {/* Completed Lessons */}
        <div className="bg-white rounded-xl border border-gray-200 p-4 flex flex-col items-center justify-center">
          <span className="text-gray-500 text-sm">{t('profile.completedLessons')}</span>
